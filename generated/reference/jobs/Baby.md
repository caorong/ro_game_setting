...
> 精确公式与实现：[查看 `Baby` 公式页](../skill-formulas/Baby.md)

## 有效技能列表

| ID | 技能 | 英文名 | 最高等级 | 来源职业 | 直接学习 | 前置 | 类型 / 目标 |
|---:|---|---|---:|---|---|---|---|
| 1 | `NV_BASIC` | Basic Skill | 9 | Baby | 是 | — | None / Passive |
| 142 | `NV_FIRSTAID` | First Aid | 1 | Baby | 是 | — | None / Self |
| 143 | `NV_TRICKDEAD` | Play Dead | 1 | Baby | 是 | — | None / Self |
| 408 | `WE_BABY` | Baby | 1 | Baby | 是 | — | None / Self |
| 409 | `WE_CALLPARENT` | Call Parent | 1 | Baby | 是 | — | None / Self |
...
- 前置技能：—
- 公式覆盖：`dedicated-source` / `source-indexed`
- 实现文件：`src/map/pc.cpp`, `src/map/skills/other/baby.cpp`, `src/map/skills/other/skill_factory_other.cpp`

公式/效果候选源码（保持原始 C++ 表达式）：

```cpp
// src/map/skills/other/baby.cpp:14
void SkillBaby::castendNoDamageId(block_list* src, block_list* target, uint16 skill_lv, t_tick tick, int32& flag) const {
// src/map/skills/other/baby.cpp:15
...
- 前置技能：—
- 公式覆盖：`dedicated-source` / `source-indexed`
- 实现文件：`src/map/pc.cpp`, `src/map/skill.cpp`, `src/map/skills/other/callparent.cpp`, `src/map/skills/other/skill_factory_other.cpp`
